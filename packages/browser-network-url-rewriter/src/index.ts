export function browserNetworkUrlRewriter(
  rewriteFn: (originalUrl: string) => string
) {
  ...
  _window.fetch = function (this: any, resource, init) {
    if (typeof resource === 'object') {
      if ('href' in resource) {
        ...
        return tempFn.call(
          this,
          new Request(rewriteFn(resource.url), resource),
          init
        )
      }
    } else {
  ...
  _window.XMLHttpRequest.prototype.open = function (...args) {
    ...
    return tempXmlOpen.call(
      this,
      _,
      rewriteFn(typeof resource === 'object' ? resource.href : resource),
      ...rest
    )
  }
